    ...
    for word in words:  # for each word
        if dictionary.get(word) is None: # If the word was not yed added to the vocabulary
            if amount_dictionary[word] < threshold:
                ...
                #dictionary[word] = tf.get_random_word_vector_bin(dimension)
            else:
            ...
            while i < length:
                if not (word_space.get(words[i]) is None):
                    ...
                    """while (i - k >= 0) and (k <= window_size): #process left neighbors of the focus word
                        ...
                        #left_neib[words[i]] = np.add(word_space[words[i]], np.roll(dictionary[words[i - k]], -1))
                        k += 1
                    ...
                    if k <= window_size and (len(lines[1])>0):
                        if len(lines[1]) < 2:
                            if k != 1: #if one word on the left was already added
                                ...
                                #left_neib[words[i]] = np.add(word_space[words[i]],
                                ...
                                #left_neib[words[i]] = np.add(word_space[words[i]],
                                                              #np.roll(dictionary[lines[1][0]], -1))
                                #left_neib[words[i]] = np.add(word_space[words[i]],
                                                              #np.roll(dictionary[lines[0][len(lines[0]) - 1]], -1))
                        else:
                            if k != 1:
                                word_space[words[i]] = np.add(word_space[words[i]],
                                ...
                                #left_neib[words[i]] = np.add(word_space[words[i]],
                                                              #np.roll(dictionary[lines[1][len(lines[1]) - 1]], -1))
                                #left_neib[words[i]] = np.add(word_space[words[i]],
                                                              #np.roll(dictionary[lines[1][len(lines[1]) - 2]], -1))"""
                    k = 1
                    while (i + k < length) and (k <= window_size): #process right neighbors of the focus word
                        ...
                        #right_neib[words[i]] = np.add(word_space[words[i]], np.roll(dictionary[words[i + k]], 1))
                        if k == 1:
                            dictionaryDouble[tuple([words[i], words[i + k]])] = tf.get_random_word_vector(dimension, ones_number)
                        k += 1
                    if k <= window_size:
                        if len(lines[3]) < 2:
                            if k != 1:
                                """word_space[words[i]] = np.add(word_space[words[i]], np.roll(dictionary[lines[3][0]], 1))""" #update word embedding
                                #right_neib[words[i]] = np.add(word_space[words[i]], np.roll(dictionary[lines[3][0]], 1))
                            else:
                                #word_space[words[i]] = np.add(word_space[words[i]], np.roll(dictionary[lines[3][0]], 1)) #update word embedding
                                #word_space[words[i]] = np.add(word_space[words[i]], np.roll(dictionary[lines[4][0]], 1)) #update word embedding
                                #right_neib[words[i]] = np.add(word_space[words[i]], np.roll(dictionary[lines[3][0]], 1))
                                #right_neib[words[i]] = np.add(word_space[words[i]], np.roll(dictionary[lines[4][0]], 1))
                                dictionaryDouble[tuple([words[i], lines[3][0]])] = tf.get_random_word_vector(dimension, ones_number)
                        else:
                            if k != 1:
                                """word_space[words[i]] = np.add(word_space[words[i]], np.roll(dictionary[lines[3][0]], 1))""" #update word embedding
                                #right_neib[words[i]] = np.add(word_space[words[i]], np.roll(dictionary[lines[3][0]], 1))
                                ...
                                #right_neib[words[i]] = np.add(word_space[words[i]], np.roll(dictionary[lines[3][0]], 1))
                                #right_neib[words[i]] = np.add(word_space[words[i]], np.roll(dictionary[lines[3][1]], 1))
                                dictionaryDouble[tuple([words[i], lines[3][0]])] = tf.get_random_word_vector(dimension, ones_number)
                i += 1
            lines.pop(0)
        line = text_file.readline()
...
#for word in word_space: #assigning zero vectors
 #   word_space[word] = np.zeros(dimension)



for word in word_space:
    for array in dictionaryDouble:
        if (array[0]== word) or (array[1] == word):
            word_space[word] = np.add(word_space[word], dictionaryDouble[array])
...
for word in word_space:
    for array in dictionaryDouble:
        if (array[0] == word) or (array[1] == word):
            pairs[word] = np.add(pairs[word], 1)
...
print "total amount of pairs: %s" % (total)


"""for word in left_neib: #checking if there are elements which are more than 1,-1
    for j in left_neib[word]:
        if j > 1:
            j = 1
        if j < -1:
            j = -1

for word in right_neib: #checking if there are elements which are more than 1,-1
    for j in right_neib[word]:
        if j > 1:
            j = 1
        if j < -1:
            j = -1
...
left_neib2 = {}
right_neib2 = {}
positions = np.random.permutation(dimension) #get random permutation
for word in left_neib: #creating dictionaries for permutated vectors
    left_neib2[word] = np.zeros(dimension)
for word in right_neib:
    right_neib2[word] = np.zeros(dimension)

for word in left_neib: #permutate vectors
    for j in range(dimension):
        left_neib2[word][j] = left_neib[word][positions[j]]
for word in right_neib:
    for j in range(dimension):
        right_neib2[word][j] = right_neib[word][positions[j]]"""

"""for word in left_neib2: #removing not zero elements after the second one
    ind = 0
    for j in range(dimension):
        if left_neib2[word][j] != 0 and ind >= 2:
            left_neib2[word][j] = 0
            ind += 1
        if left_neib2[word][j] != 0 and ind < 2:
            ind += 1"""

"""for word in right_neib2: #removing not zero elements after the second one
    ind = 0
    for j in range(dimension):
        if right_neib2[word][j] != 0 and ind >= 2:
            right_neib2[word][j] = 0
            ind += 1
        if right_neib2[word][j] != 0 and ind < 2:
            ind += 1"""
"""for word in left_neib2:
    ind1 = 0
    ind2 = 0
    for j in range(dimension):
        if left_neib2[word][j] == -1 and ind1 > 0:
            left_neib2[word][j] = 0
            ind1 += 1
        if left_neib2[word][j] == -1 and ind1 == 0:
            ind1 += 1
        if left_neib2[word][j] == 1 and ind2 > 0:
            left_neib2[word][j] = 0
            ind2 += 1
        if left_neib2[word][j] == 1 and ind2 == 0:
            ind2 += 1

for word in right_neib2:
    ind1 = 0
    ind2 = 0
    for j in range(dimension):
        if right_neib2[word][j] == -1 and ind1 > 0:
            right_neib2[word][j] = 0
            ind1 += 1
        if right_neib2[word][j] == -1 and ind1 == 0:
            ind1 += 1
        if right_neib2[word][j] == 1 and ind2 > 0:
            right_neib2[word][j] = 0
            ind2 += 1
        if right_neib2[word][j] == 1 and ind2 == 0:
            ind2 += 1
...
for word in left_neib2: #adding obtained vectors
    word_space[word] = np.add(word_space[word], left_neib2[word])
for word in right_neib2:
    word_space[word] = np.add(word_space[word], right_neib2[word])"""
